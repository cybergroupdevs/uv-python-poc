    ...
    def discount(transactionId):
        ...
        if(status):
            ...
            cmd = "LIST DATA RESERVATIONS DISCOUNT.TYPE COUPON.CODE DISCOUNT.POD.TYPE DISCOUNT.POD.AMT DISCOUNT.EMPLOYEE.ID DISCOUNT.REASON.CODE DISCOUNT.REASON.TEXT PROMOS.ID IN.ADDITION.TO.MRKDN DISC.CNV.MRKDN ADVERT.CODE CORP.ACCT.NO LINE.DISCOUNT QUANTITY {} TRANSACTION TOJSON".format(
                transaction_id)
            details = u2py.Command(cmd).run(capture=True)
            details = json.loads(details)
            if(presale_detail != 'Y'):
                count = (
                    count((details['TRANSACTION'][0]['DISCOUNT.TYPE_MV'][0]['DISCOUNT.TYPE'])))
                ...
                for x in range(1, total_count):
                    ...
                    for y in range(1, n_lines):
                        scr_discount = scr_discount+((details['TRANSACTION'][0]['ITEMS_MV'][y]['RESERVATIONS'])*(
                            details['TRANSACTION'][y]['ITEMS_MV'][x]['LINE.DISCOUNT']))
                        continue
                        discount = discount+(details['TRANSACTION'][y]['ITEMS_MV'][0]['QUANTITY'])*(
                            details['TRANSACTION'][y]['ITEMS_MV'][x]['LINE.DISCOUNT'])
                    ...
                    if(disc_type == 1):
                        ...
                        kgm_cmd = "LIST DATA LNAME FNAME COMPANY.CODE {} ToJSON".format(
                            emp_id)
    ...
    def refund(transactionId):
        ...
        if(status):
            data_file = u2py.File("TRANSACTION")
            cmd = "LIST DATA NICKNAME LIKE.TENDER.OVERRIDE.MGR.ID FNAME LNAME SHORTNAME HAND.TKT.NO {} TRANSACTION".format(
                transaction_id)
            details = u2py.Command(cmd).run(capture=True)
            details = json.loads(details)
            ...
            refund_mgr_name = refund_mgr_name + ' ' + \
                (details['EM'][0]['LNAME']) + \
                ' ('+(details['EM'][0]['SHORTNAME'])+')'
            refund_data = {}
