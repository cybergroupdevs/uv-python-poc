...
def discount(transactionId):
    ...
    if(status):
        ...
        cmd="LIST DATA RESERVATIONS DISCOUNT.TYPE COUPON.CODE DISCOUNT.POD.TYPE DISCOUNT.POD.AMT DISCOUNT.EMPLOYEE.ID DISCOUNT.REASON.CODE DISCOUNT.REASON.TEXT PROMOS.ID IN.ADDITION.TO.MRKDN DISC.CNV.MRKDN ADVERT.CODE CORP.ACCT.NO LINE.DISCOUNT QUANTITY {} TRANSACTION TOJSON".format(transaction_id)
        details=u2py.Command(cmd).run(capture=True)
        details=json.loads(details)
        if(presale_detail != 'Y'):
            ...
            for x in range(1,total_count):
                ...
                if(disc_type==1):
                    ...
                    kgm_cmd="LIST DATA LNAME FNAME COMPANY.CODE {} ToJSON".format(emp_id)
                    ...
                    pct=pct+' ('+kgm_details["KG.EM"]["0"]["FNAME"]+' '+kgm_details["KG.EM"]["0"]["LNAME"]+')'
                elif(disc_type==10):
                    pct='Affiliate employee family'
                    pct=kgm_details["KG.EM"]["0"]["COMPANY.CODE"]+' employee family'
                    pct=pct+' (of '+kgm_details["KG.EM"]["0"]["FNAME"]+' '+kgm_details["KG.EM"]["0"]["LNAME"]+')'
                elif(disc_type==11):
...
def refund(transactionId):
    ...
    if(status):
        data_file = u2py.File("TRANSACTION")
        cmd="LIST DATA NICKNAME LIKE.TENDER.OVERRIDE.MGR.ID FNAME LNAME SHORTNAME HAND.TKT.NO {} TRANSACTION".format(transaction_id)
        details=u2py.Command(cmd).run(capture=True)
        details=json.loads(details)
        ...
        for x in range(0,n_lines):
            ...
            if(presale_detail != 'Y'):
                ...
                if(item_ship_group==curr_ship_group):
                    sale_total = 0
                    line_total=(float(details['TRANSACTION'][0]['ITEM_MV'][x]['RETAIL'])-float(details['TRANSACTION'][0]['ITEM_MV'][x]['LONG.MRKDN']))*int(details['TRANSACTION'][0]['ITEM_MV'][x]['QUANTITY'])
            ...
            description_output=[]
            extCommOut=''
            desc = data_file.readv(details['TRANSACTION'][0]['_ID'],14).dcount(u2py.SM)
            if(desc == 1):
                if(presale_detail=='Y'):
                    emp_id=int(float(details['TRANSACTION'][0]['ITEM_MV'][x]['CommEmplId']))
                ...
                if(rental_id != ''):
                    rental=' (Rental '+rental_id+' for'+((details['TRANSACTION'][0]['ITEM_MV'][x]['TUX.RENTAL.AMT'])*100)+', '+((details['TRANSACTION'][0]['ITEM_MV'][x]['TUX.INSURANCE.AMT'])*100)+' ins, '+((details['TRANSACTION'][0]['ITEM_MV'][x]['TUX.RUSH.AMT'])*100)+' rush'
                    ...
                    if (details['TRANSACTION'][0]['ITEM_MV'][x]['TUX.MARKDOWN.AMT'] != ''):
                        rental=rental+', '+((details['TRANSACTION'][0]['ITEMS_MV'][x]['TUX.MARKDOWN.AMT'])*100)+' mkdn'
                    rental=rental+')'
                else:
                    unconverted_dist_total=0
                line_total=(float(details['TRANSACTION'][0]['ITEM_MV'][x]['RETAIL'])-(float(details['TRANSACTION'][0]['ITEM_MV'][x]['MRKDN'])))*(int(details['TRANSACTION'][0]['ITEM_MV'][x]['QUANTITY']))
                line_total=line_total-unconverted_disc_amount
                if(x> total_accumulated_line):
                    sale_total=sale_total+line_total
                    total_accumulated_line=total_accumulated_line+1
